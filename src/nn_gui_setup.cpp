...
void NN_gui_setup::display_int_setup()
{
	...
	//window.setView(view);
	window.create(sf::VideoMode(win_x * screenScalingFactor, win_y * screenScalingFactor), "Neural Netowrk Interactive Setup");
...
void NN_gui_setup::display_title()
{
	...
	// Set font attributes for lower text
	sf::Color c(100,100,100,125);
	load_network.setFont(title_font);
	load_network.setString("\n\n3: Load network");
	load_network.setCharacterSize(20);
	load_network.setFillColor(c);
	load_network.setStyle(sf::Text::Bold | sf::Text::StrikeThrough);
	load_network.setPosition(20,opt_text.getPosition().y + opt_text.getGlobalBounds().height);
